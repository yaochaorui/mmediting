model = dict(
    ...
    backbone=dict(
        ...
        encoder=dict(
            type='FBAResnetDilated', depth=50,in_channels=11,
                 stem_channels=64,
                 base_channels=64, conv_cfg=dict(type='ConvWS'),norm_cfg=dict(type='GN',num_groups=32),act_cfg=dict(type='ReLU',inplace=True)),
        decoder=dict(
            type='FBADecoder', pool_scales=(1, 2, 3, 6), in_channels=2048, channels=256, conv_cfg=dict(type='ConvWS'),norm_cfg=dict(type='GN',num_groups=32),act_cfg=dict(type='LeakyReLU',inplace=False))),
...
train_pipeline = [
    ...
    dict(
        type='Collect',
        keys=[
            'merged','trimap_transformed','two_channel_trimap','merged_unnormalized', 'alpha', 'fg', 'bg',
            'ori_fg','two_channel_trimap'
        ],meta_keys=['pad']),
    dict(
        type='ImageToTensor',
        keys=[
...
test_pipeline = [
    ...
    dict(
        type='Collect',
        keys=['merged','trimap_transformed','two_channel_trimap','merged_unnormalized'],
        meta_keys=[
